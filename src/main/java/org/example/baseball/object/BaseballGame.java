...
public class BaseballGame { // 게임 객체는 투수와 타자, 심판, 최종결과(횟수) 그리고 조이스틱(입출력)으로 구성되고 각각의 객체는 협력이 불가피한 관계
    ...
    public void playGame() {
        ...
        // 투수 공 생성(난수)
        pitcher.generateRandomNumbers();

        while (true) {
            // 사용자 방망이질 (입력장치를 통해 숫자 입력)
            String input = batter.getInput();

            // 심판이 유효성 체크
            if (!umpire.validateInput(input)) {
                inputOutputHandler.printMessage("잘못된 입력: 1부터 9까지의 서로 다른 숫자로 이루어진 3자리 수 입력");
                continue;
            }

            // 심판이 유효한 볼인지 최종 확인
            int[] userNumbers = umpire.parseInput(input);
            attempts++; // 횟수 증가

            // 심판이 결과 판독
            int[] result = umpire.compareNumbers(pitcher.getNumbers(), userNumbers);
