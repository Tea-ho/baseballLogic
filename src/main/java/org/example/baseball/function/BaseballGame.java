...
public class BaseballGame {
    ...
    public static void playGame() {
        ...
        // 1. 스캐너 생성
        InputOutputHandler inputOutputHandler = new BaseballGameInputOutputHandler();

        // 2. 컴퓨터 숫자 / 유저 숫자 받을 공간 만들기
        RandomNumberGenerator computerNumberGenerator = new BaseballGameComNumberGenerator();
        ...
        while (true) {
            ...
            // 5-2. 유효성 검사 처리
            if ( !BaseballGameValidate.isValid(input) ) {
                inputOutputHandler.printMessage("잘못된 입력: 1부터 9까지의 서로 다른 숫자로 이루어진 3자리 수 입력");
                continue;
            }

            // 5-3. 유저 번호
            userNumbers = BaseballGameTypeCasting.parsingInput(input);
            attempts++;

            // 5-4. 결과 판독
            int[] result = BaseballGameResultHandler.compareNumbers(computerNumbers, userNumbers);
